...
const fetchTotalUsers = async () => {
  try {
    const res = await axios.get('http://localhost:3000/api/admin/total-users')
...
const updateGOR = async () => {
  ...
  try {
    ...
    await axios.put(`http://localhost:3000/api/admin/gor/${editForm.value.id_gor}`, payload);
...
const deleteGOR = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:3000/api/admin/gor/${id}`)
...
const fetchDSSStats = async () => {
  try {
    const res = await axios.get('http://localhost:3000/api/admin/dss-stats')
